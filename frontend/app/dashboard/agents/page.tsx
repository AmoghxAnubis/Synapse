...
import { Bot, Plus, Settings2, FileText, Globe, Code, BrainCircuit, Trash2, Search, TerminalSquare, Database, Loader2, X, Send, Plug, GitPullRequest, MessageSquare, BookOpen, LayoutGrid, Gamepad2 } from "lucide-react";
...
/* ── Integration definitions for the Context Pool ──── */
...
interface IntegrationDef {
    ...
    activeColor: string;       // tailwind bg color when active
    activeBorder: string;      // tailwind border color when active
    activeText: string;        // tailwind text color when active
    badgeBg: string;           // for header badge
}
...
const INTEGRATION_DEFS: IntegrationDef[] = [
    {
        id: "github",
        name: "GitHub",
        description: "Pull repos, PRs, issues & commits",
        icon: <GitPullRequest className="h-5 w-5" />,
        activeColor: "bg-purple-50 dark:bg-purple-900/20",
        activeBorder: "border-purple-300 dark:border-purple-700",
        activeText: "text-purple-600 dark:text-purple-400",
        badgeBg: "bg-purple-50 text-purple-600 dark:bg-purple-900/20 dark:text-purple-400",
    },
    {
        id: "notion",
        name: "Notion",
        description: "Sync pages, databases & blocks",
        icon: <BookOpen className="h-5 w-5" />,
        activeColor: "bg-stone-50 dark:bg-stone-900/20",
        activeBorder: "border-stone-300 dark:border-stone-600",
        activeText: "text-stone-700 dark:text-stone-300",
        badgeBg: "bg-stone-50 text-stone-700 dark:bg-stone-900/20 dark:text-stone-300",
    },
    {
        id: "jira",
        name: "Jira",
        description: "Access projects, issues & sprints",
        icon: <LayoutGrid className="h-5 w-5" />,
        activeColor: "bg-blue-50 dark:bg-blue-900/20",
        activeBorder: "border-blue-300 dark:border-blue-700",
        activeText: "text-blue-600 dark:text-blue-400",
        badgeBg: "bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400",
    },
    {
        id: "slack",
        name: "Slack",
        description: "Read channels, messages & threads",
        icon: <MessageSquare className="h-5 w-5" />,
        activeColor: "bg-emerald-50 dark:bg-emerald-900/20",
        activeBorder: "border-emerald-300 dark:border-emerald-700",
        activeText: "text-emerald-600 dark:text-emerald-400",
        badgeBg: "bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-400",
    },
    {
        id: "discord",
        name: "Discord",
        description: "Read guilds, channels & messages",
        icon: <Gamepad2 className="h-5 w-5" />,
        activeColor: "bg-indigo-50 dark:bg-indigo-900/20",
        activeBorder: "border-indigo-300 dark:border-indigo-700",
        activeText: "text-indigo-600 dark:text-indigo-400",
        badgeBg: "bg-indigo-50 text-indigo-600 dark:bg-indigo-900/20 dark:text-indigo-400",
    },
];
...
export default function AgentsPage() {
    ...
    const [editedIntegrations, setEditedIntegrations] = useState<string[]>([]);
    ...
    const [integrationStatuses, setIntegrationStatuses] = useState<Record<string, IntegrationStatusEntry>>({});
    ...
    useEffect(() => {
        ...
        // Fetch integration connection statuses once
    ...
    const handleSave = async () => {
        ...
        try {
            const updated = await updateAgent(selectedAgent.id, {
                ...
                integrations: editedIntegrations,
            });
                    ...
                    <>
                        <header className="px-8 py-6 border-b border-neutral-200 dark:border-neutral-800 flex justify-between items-start">
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2 mt-2">
                                    ...
                                    {editedIntegrations.length > 0 && (
                                        <span className="text-[10px] font-semibold bg-teal-50 text-teal-600 dark:bg-teal-900/20 dark:text-teal-400 px-2 py-0.5 rounded-full">
                                            🔌 {editedIntegrations.length} integration{editedIntegrations.length > 1 ? 's' : ''}
                                        </span>
                                    )}
                                </div>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-8 max-w-4xl">
                            ...
                            <section className="mb-10">
                                ...
                                <p className="text-sm text-neutral-500 dark:text-neutral-400 mb-4">
                                    Connect this agent to external platforms. It will be able to pull live context from enabled integrations during conversations.
                                </p>
                                ...
                                <div className="border border-neutral-200 dark:border-neutral-700 rounded-lg overflow-hidden">
                                    ...
                                    <div className="p-4 bg-neutral-50 dark:bg-neutral-800/50 border-b border-neutral-200 dark:border-neutral-700 flex justify-between items-center">
                                        <span className="text-sm font-medium text-neutral-700 dark:text-neutral-300">
                                            {editedIntegrations.length === 0
                                                ...
                                                : `${editedIntegrations.length} integration${editedIntegrations.length > 1 ? 's' : ''} active`}
                                        </span>
                                        {editedIntegrations.length > 0 && (
                                            <button
                                                onClick={() => setEditedIntegrations([])}
                                    ...
                                    <div className="divide-y divide-neutral-100 dark:divide-neutral-800">
                                        {INTEGRATION_DEFS.map((integ) => {
                                            const isActive = editedIntegrations.includes(integ.id);
                                            const status = integrationStatuses[integ.id];
                                                ...
                                                >
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div className={cn(
                                                            ...
                                                            isActive ? integ.activeColor : "bg-neutral-100 dark:bg-neutral-800"
                                                        )}>
                                                            <span className={cn(isActive ? integ.activeText : "text-neutral-500 dark:text-neutral-400")}>
                                                                {integ.icon}
                                                            </span>
                                                        </div>
                                                        {/* Label + description */}
                                                            ...
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                <span className={cn(
                                                                    ...
                                                                    <span className={cn(
                                                                        ...
                                                                        isConnected ? "bg-green-500" : "bg-neutral-300 dark:bg-neutral-600"
                                                                    )} />
                                                                    {isConnected ? "Connected" : "Not connected"}
                                                                </span>
                                                            </div>
                                                            <div className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">{integ.description}</div>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className={cn(
                                                        ...
                                                        isActive
                                                            ? `${integ.activeBorder} ${integ.activeColor}`
                                                            : "border-neutral-300 dark:border-neutral-600"
                                                    )}>
                                                        {isActive && (
                                                            <svg className={cn("w-3 h-3", integ.activeText)} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
                                    ...
                                    <div className="px-4 py-3 bg-neutral-50 dark:bg-neutral-800/50 border-t border-neutral-100 dark:border-neutral-800">
                                        <p className="text-[11px] text-neutral-400 dark:text-neutral-500 flex items-center gap-1.5">
                                            ...
                                            Manage API keys and connection settings in <a href="/settings/integrations" className="underline hover:text-neutral-600 dark:hover:text-neutral-300 transition-colors">Settings → Integrations</a>
                                        </p>
                                    </div>
                                </div>
                            </section>
