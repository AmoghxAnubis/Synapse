...
export default function NeuralMesh() {
    ...
    useFrame((state) => {
        ...
        for (let i = 0; i < PARTICLE_COUNT; i++) {
            ...
            const dx = mx - arr[ix];
            const dy = my - arr[iy];
            // ⚡ Bolt: Optimize by using squared distance to avoid expensive Math.sqrt calls (~150x per frame)
            const distSq = dx * dx + dy * dy;

            if (distSq < MOUSE_RADIUS_SQ) {
                const dist = Math.sqrt(distSq); // Only calculate sqrt when strictly necessary for the force equation
        ...
        for (let i = 0; i < PARTICLE_COUNT; i++) {
            for (let j = i + 1; j < PARTICLE_COUNT; j++) {
                ...
                // ⚡ Bolt: Use squared distance to skip Math.sqrt for over 11,000 potential connections per frame
